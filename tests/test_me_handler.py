"""Tests for MeHandler: user profile with rewards."""
...
class TestMeHandler:
    async def test_profile_no_rewards(self):
        ...
        reply, ok = await invoke(MeHandler, "/me", repo, user_id=USER_ID)
        assert ok
        assert "42" in reply
        assert "нет" in reply  # no achievements
    ...
    async def test_profile_unknown_user(self):
        ...
        reply, ok = await invoke(MeHandler, "/me", repo, user_id=USER_ID)
        assert ok
        assert "0" in reply  # monkeys default
