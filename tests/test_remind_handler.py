"""Tests for remind handlers: add, remove, list."""
import datetime
...
class TestRemindersHandler:
    async def test_empty(self):
        from steward.handlers.remind_handler import RemindersHandler

        repo = make_repository()
        reply, ok = await invoke(RemindersHandler, "/reminders", repo)
        assert ok
        assert "нет" in reply
    ...
    async def test_accessible_via_remind_list(self):
        ...
        reply, ok = await invoke(RemindersHandler, "/remind list", repo)
        assert ok
        assert "позвонить" in reply


class TestRemindRemoveHandler:
    async def test_removes_reminder(self):
        from steward.handlers.remind_handler import RemindRemoveHandler

        repo = make_repository()
        repo.db.delayed_actions = [_reminder("abc1", "купить молоко")]
        reply, ok = await invoke(RemindRemoveHandler, "/remind remove abc1", repo)
        assert ok
    ...
    async def test_not_found(self):
        ...
        repo = make_repository()
        reply, ok = await invoke(RemindRemoveHandler, "/remind remove xyz", repo)
        assert ok
        assert "не найдено" in reply


class TestRemindEditHandler:
    async def test_edits_reminder(self):
        from steward.handlers.remind_handler import RemindEditHandler

        repo = make_repository()
        repo.db.delayed_actions = [_reminder("abc1", "старый текст")]
        reply, ok = await invoke(RemindEditHandler, "/remind edit abc1 новый текст", repo)
        assert ok
    ...
    async def test_not_found(self):
        ...
        repo = make_repository()
        reply, ok = await invoke(RemindEditHandler, "/remind edit xyz новый текст", repo)
        assert ok
        assert "не найдено" in reply


class TestRemindAddHandler:
    async def test_adds_reminder_with_interval(self):
        from steward.handlers.remind_handler import RemindAddHandler

        repo = make_repository()
        reply, ok = await invoke(RemindAddHandler, "/remind 10m купить молоко", repo)
        assert ok
        assert "✅" in reply
    ...
    async def test_invalid_time(self):
        ...
        repo = make_repository()
        reply, ok = await invoke(RemindAddHandler, "/remind notatime купить молоко", repo)
        assert ok
    ...
    async def test_no_text(self):
        ...
        repo = make_repository()
        reply, ok = await invoke(RemindAddHandler, "/remind 10m", repo)
        assert ok
