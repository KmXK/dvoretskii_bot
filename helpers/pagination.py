...
def create_pagination_keyboard(
    ...
    max_page_number = page_count - 1

    buttons = [
        ["<", current_page - 1 if current_page > 0 else 0],
        [f"{current_page + 1}/{page_count}", current_page],
...
def get_data_page(
    ...
    return (
        ...
        create_pagination_keyboard(
            unique_keyboard_name,
            current_page=page,
            page_count=ceil(length / page_size),
        )
...
class Paginator:
    ...
    async def show_list(self, update: Update):
        text, keyboard = self._get_data_page(page=0)
    ...
    def _get_data_page(self, page: int):
