...
async def make_openrouter_query(
    ...
    def _call():
        return openrouter_client.chat.completions.create(  # type: ignore[union-attr]
            model=model,
    ...
    if not content:
        raise ValueError(f"Model returned empty response (finish_reason={response.choices[0].finish_reason})")
    return content
