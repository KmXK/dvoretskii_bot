...
class RemindAddHandler(Handler):
    ...
    def prompt(self) -> str | None:
        return (
            ...
            "  Синтаксис: /remind <время> [x<кол-во>] [дни] <текст>\n"
            "  Порядок аргументов строгий: СНАЧАЛА время, ПОТОМ повторение, ПОТОМ дни, ПОТОМ текст.\n"
            "  Время (первый аргумент, обязательный):\n"
            "  - Интервал: 10m, 2h30m, 1d, 45s\n"
            "  - Время суток: 9:00, 15:30\n"
            "  - Дата и время: 25.12 10:00\n"
            "  Повторение (необязательный):\n"
            "  - x3 — повторить 3 раза\n"
            "  - x* — повторять бесконечно\n"
            "  - Если пользователь говорит «каждый», «ежедневно», «еженедельно» — используй x*\n"
            "  Дни недели (необязательный, только короткие имена):\n"
            "  - Русские: пн, вт, ср, чт, пт, сб, вс\n"
            "  - Английские: mon, tue, wed, thu, fri, sat, sun\n"
            "  - Группы: weekday (пн-пт), weekend (сб-вс)\n"
            "  - НЕ используй полные названия (monday, tuesday и т.д.)\n"
            "  Примеры:\n"
            "  - «напомни через 10 минут позвонить» → /remind 10m позвонить\n"
            "  - «каждый понедельник в 9 утра намаз» → /remind 9:00 x* пн намаз\n"
            "  - «каждый будний день в 8:30 зарядка» → /remind 8:30 x* weekday зарядка\n"
            "  - «25 декабря в 10:00 поздравить» → /remind 25.12 10:00 поздравить\n"
            "  - «каждый день в 22:00 выпить воду» → /remind 22:00 x* выпить воду\n"
            "  - «напомни завтра в 15:30 встреча» → /remind 15:30 встреча"
        )
