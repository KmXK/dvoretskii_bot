...
class CurseFeature(Feature):
    ...
    def _is_curse_admin(self, ctx: FeatureContext) -> bool:
        # Управление матами/наказаниями gated на право curse.manage
        # (роль «Идеолог», создаётся миграцией). Глобал-админ проходит всегда.
        return ctx.repository.has_permission(ctx.user_id, "curse.manage")
