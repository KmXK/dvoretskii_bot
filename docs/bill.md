...
- если не найден, ищет в той же папке любой файл с именем "Копия Шаблон" и переименовывает найденный файл в имя счета;
- если шаблонов нет, возвращает ошибку;
после этого выдает "начальный отчет" по счету

/bill add {имя} — добавление счета одной командой (без сессии), с той же логикой переименования "Копия Шаблон"
...
/bill {id} edit - запустить сессию сбора контекста для счета
бот просит отправлять контекст (фото, голосовые, текст), после кнопки "Стоп" весь собранный контекст отправляется в нейронку и добавляется в счет новыми строками (без перезаписи)
...
- блок с платежными данными для пользователей которым должны(лишних не надо)
- ссылка на файл в гугл драйве
- inline-кнопка "Добавить контекст" для запуска сессии сбора контекста

"сессия сбора контекста по счету":
- принимает несколько сообщений подряд
- фото: OCR (Yandex) -> текст
- голосовые: транскрибация (ElevenLabs) -> текст
- текстовые сообщения тоже принимаются как контекст
- завершение по кнопке "Стоп" (или /stop)
- после завершения весь собранный текст отправляется в AI-парсер
- перед отправкой боту также читается второй лист "Данные" (персонаж|места), и этот список передается в AI как текущий контекст участников
- из листа "Данные" дополнительно извлекается список известных мест (если есть отдельный столбец/секция "Места"), чтобы AI выбирал место из справочника
- AI возвращает 2 блока:
  - [ОБЩЕЕ] -> строки для основного листа "Общее" с колонками:
    Наименование | Достоинство | Плательщик | Плательщик (фактический) | Происхождение
  - [ДАННЫЕ] -> только новые участники для второго листа "Данные":
    персонаж | места
- бот добавляет новые строки вставкой (без перезаписи) в оба листа
- служебные имена спикеров из diarization (например "Ратибор") не должны попадать в список новых участников
...
свободная переплата всегда показывается отдельно; при появлении подходящего подтвержденного долга она автоматически уменьшает его по FIFO, а свободный остаток уменьшается
внутри счета показывается точная сумма, которая была оплачена из переплаты, в том числе после закрытия счета
...
актуальный формат листа "Общее" (используется при разборе контекста):
Наименование|Достоинство|Плательщик|Плательщик (фактический)|Происхождение

лист "Данные":
персонаж|места
- может содержать пустые "места"
- используется как справочник известных участников и мест
