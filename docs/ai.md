...
1. Ловит сообщения начинающиеся с триггера (не команды `/`)
2. Если сообщение — реплай, текст и информация об отправителе `reply_to_message` передаётся LLM как контекст (включая `forward_origin` для пересланных сообщений)
