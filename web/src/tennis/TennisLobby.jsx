...
function SessionRow({ s, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl bg-zinc-900/70 hover:bg-zinc-800 border border-zinc-800 text-left"
    >
      <span className="text-zinc-500 text-xs font-mono w-12 shrink-0">#{s.id}</span>
      <span className="text-zinc-300 text-xs font-mono w-16 shrink-0">{date}</span>
      <span className="text-white truncate flex-1 text-sm">
        {s.player_a_name} <span className="font-mono tabular-nums">{a}:{b}</span> {s.player_b_name}
      </span>
      <span className={`text-xs px-1.5 py-0.5 rounded ${isLive ? 'bg-emerald-700/60 text-emerald-100' : 'bg-zinc-800 text-zinc-400'}`}>
        {tag}
      </span>
    </button>
  )
...
export default function TennisLobby({ onStartLive, onOpenSession, onOpenImport, onOpenStats, onOpenNewSession }) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen px-4 pt-6 pb-8 max-w-2xl mx-auto"
    >
      ...
      {liveSession && (
        <button
          onClick={onStartLive}
          className="w-full mb-4 bg-gradient-to-br from-emerald-600 to-emerald-800 text-white rounded-xl px-4 py-3 text-left shadow-lg active:scale-[0.98] transition-transform"
        >
          <div className="text-[10px] uppercase tracking-wider opacity-80">Активная сессия</div>
          <div className="font-semibold mt-0.5">
            {liveSession.player_a_name} {liveSession.wins[0]}:{liveSession.wins[1]} {liveSession.player_b_name}
          </div>
          <div className="text-xs opacity-80 mt-0.5">Открыть табло →</div>
        </button>
      )}

      <button
        onClick={onOpenNewSession}
        className="w-full mb-3 bg-gradient-to-br from-rose-600 to-rose-800 text-white rounded-xl px-4 py-4 font-semibold text-lg shadow-lg active:scale-[0.98] transition-transform"
      >
        ▶ Начать сессию
      </button>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <button
          onClick={onOpenStats}
          className="bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl px-4 py-3 border border-zinc-800 text-left"
        >
          <div className="text-2xl mb-1">📊</div>
          <div className="font-semibold text-sm">Статистика</div>
        </button>
        <button
        ...
        >
          <div className="text-2xl mb-1">📥</div>
          <div className="font-semibold text-sm">Импорт</div>
        </button>
      </div>
